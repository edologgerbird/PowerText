...
def run():
    '''This function runs the content explorer app.
    '''
    ...
    if util.check_session_state_key("csv_file"):
        ...
        with col1:
            subreddit = st.selectbox(
                "Select Subreddit", content_explorer.get_unique_subreddits(reddit_content))
        with col2:
            st.write("")
            st.write("")
        ...
        if generate_random:
            content_explorer.show_random_post_from_subreddit(
